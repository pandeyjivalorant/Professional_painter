...
import { PaintingCard, ImageModal } from '@/components/UI';

export const CATEGORIES = ["All", "Acrylic", "Watercolor", "Oil on Canvas", "Mixed Media", "Dry Ink", "Pencil", "Pen Sketch", "Oil Pastels", "Charcoal"];
export const STYLES = ["All Styles", "Contemporary", "Abstract", "Realism", "Expressionist", "Traditional", "Impressionist"];
export const SORT_OPTIONS = ["Featured", "Price: Low to High", "Price: High to Low", "Newest", "Highest Rated"];

export default function GalleryClient({ initialPaintings }) {
  ...
  const filtered = useMemo(() => {
    ...
    if (availableOnly) list = list.filter(p => p.status === 'AVAILABLE' || p.available);
    ...
    switch (sort) {
      ...
      default: list.sort((a, b) => (b.isFeatured || b.featured ? 1 : 0) - (a.isFeatured || a.featured ? 1 : 0));
    }

    return list;
  }, [initialPaintings, search, category, style, sort, availableOnly, priceRange]);
    ...
    <div className="bg-obsidian min-h-screen pt-20">
        ...
        <div className="max-w-7xl mx-auto px-6 lg:px-10">
          <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }}>
            ...
            <p className="text-muted max-w-lg">
              {initialPaintings.length} original artworks — oils, watercolors, and mixed media. Each signed and authenticated.
            </p>
          </motion.div>
        </div>
      </div>
      ...
      <div className="max-w-7xl mx-auto px-6 lg:px-10 py-10">
        ...
        <div className="flex flex-wrap gap-4 mb-6 items-center">
          ...
          {/* Category tabs */}
          <div className="hidden md:flex gap-1 overflow-x-auto pb-2 -mb-2">
            {CATEGORIES.map(cat => (
              <button
                key={cat}
                onClick={() => setCategory(cat)}
                className={`px-4 py-2.5 text-xs tracking-[0.15em] whitespace-nowrap uppercase transition-all ${category === cat ? 'bg-gold text-obsidian' : 'border border-gold/20 text-muted hover:text-gold hover:border-gold/40'}`}
              >
                {cat}
              </button>
            ))}
